...
export default class Radial {
  constructor({ pos, radius = 100, len = 10, spokes = 50, color = 0, strokeWeight = 2, skewAngle = 0 } = {}) {
  ...
  draw() {
    ...
    const outerRad = this.radius + this.len;
    ...
    for (let i = 0; i < this.spokes; i++) {
      ...
      const x1 = this.radius * p5.cos(angle);
      const y1 = this.radius * p5.sin(angle);
      // (p5.map(angle - Math.PI, 0, Math.PI, 0, this.len));
      const x2 = (outerRad * p5.cos(angle)) + offset.x;
      const y2 = (outerRad * p5.sin(angle)) + offset.y;
